...
//	b = color & 0xFF;
//}
//int findClosestVGAColorIndex(uint8_t r, uint8_t g, uint8_t b) {
//	int closestIndex = 0;
//	uint32_t closestDistance = std::numeric_limits<uint32_t>::max();
//
//	for (int i = 0; i < 256; i++) {
//		uint8_t vgaR, vgaG, vgaB;
//		extractRGB(vga_palette[i], vgaR, vgaG, vgaB);
//
//		uint32_t distance = (r - vgaR) * (r - vgaR) +
//			(g - vgaG) * (g - vgaG) +
//			(b - vgaB) * (b - vgaB);
//
...
//
//	unsigned char* img = stbi_load(cstr, &width, &height, &channels, 4);
//	if (img == NULL) {
...
//	int x = 0;
//	int y = 0;
//	cout << "Image loaded with width = " << width << " height = " << height << " and channels = " << channels << endl;
//	ofstream fout;
//	fout.open("BinaryArr.txt", ios::out);
//	int size;
//	int index;
